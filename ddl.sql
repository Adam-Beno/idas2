...
CREATE OR REPLACE VIEW DECORATION_WITH_SCAN AS SELECT * FROM ( SELECT  b.ID, b.BOOKS_ID, b.DECORATIONS_ID, b.NAME, S.PHOTO, p.NICKNAME, ROW_NUMBER() OVER (PARTITION BY b.ID ORDER BY s.DECORATIONS_ID DESC) AS rn FROM DECORATIONS b JOIN SCANS s ON b.ID = s.DECORATIONS_ID JOIN PAINTERS p ON b.ID = p.DECORATIONS_ID) WHERE rn = 1;
...
CREATE OR REPLACE PROCEDURE SEED_USERS AS
  v_admin_id NUMBER;
  v_role_admin_id NUMBER;
  v_employee_role NUMBER;
BEGIN
  INSERT INTO ROLES (NAME, "LEVEL") VALUES ('Administrator', 0) RETURNING ID INTO v_role_admin_id;
  INSERT INTO ROLES (NAME, "LEVEL") VALUES ('Employee', 10) RETURNING ID INTO v_employee_role;

  INSERT INTO USERS (USERNAME, PASSWORD) VALUES ('admin', 1234) RETURNING ID INTO v_admin_id;
  INSERT INTO USERS (USERNAME, PASSWORD) VALUES ('employee', 1234);

  UPDATE ROLE_USER SET ROLES_ID = v_role_admin_id WHERE ROLES_ID = v_employee_role AND USERS_ID = v_admin_id;
END SEED_USERS;
/
