...
const motive = class MotiveEdit extends Component {
  ...
  async getData(id) {
    let result = await knex('MOTIVE').where('ID', id).select().first();
  ...
  async handleSubmit() {
    ...
    await knex('MOTIVE').where('ID', Number(this.props.match.params.id)).update(_omit(_merge(oldData, formData), ['DATA', 'VALUES', 'ID']));
